...
public class Hub2 extends AbstractLevel {

	// which layers to render
	private int[] layer1 = {0};
	
	// level triggers
	public Rectangle level3;

	public Hub2(Player player, SpriteBatch batch) 
	{		
		...
		map = load.load("maps/TheHub2/theHub2.tmx");
		...
		level3 = new Rectangle(collision.getWidth() * collision.getTileWidth() - 320, 27*32, 6*32, 6*32);
	}

	public Screens checkTriggers()
	{
		if(Intersector.overlaps(level3, player.bounds)) {
			loaded = false;
			return Screens.LEVEL2;
		}
		else return Screens.HUB2;
	}
