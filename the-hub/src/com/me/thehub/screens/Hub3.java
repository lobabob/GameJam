...
public class Hub3 extends AbstractLevel {
	...
	public Hub3(Player player, SpriteBatch batch) 
	{		
		...
		map = load.load("maps/TheHub3/TheHub3.tmx");
		...
		level3boss = new Rectangle(collision.getWidth() * collision.getTileWidth() - 6*32, 2*32, 32, 64);
	}
	...
	public Screens checkTriggers()
	{
		if(Intersector.overlaps(level3boss, player.bounds)) {
			loaded = false;
			return Screens.LEVEL3;
		}
		else return Screens.HUB3;
	}
	...
	public void show() { 
		if(!loaded) {
			...
			player.setStart(64, 32, new Vector2(1, 0));
			loaded = true;
		}
